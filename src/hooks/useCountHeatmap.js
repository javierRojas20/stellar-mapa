...
/**
 * Hook para obtener datos del heatmap de conteo desde Supabase RPC
 ...
 */
export const useCountHeatmap = (isActive = false, bounds = null, limit = 500) => {
  ...
  useEffect(() => {
    ...
    const fetchHeatmapData = async () => {
      try {
        ...
        console.log('🔍 Fetching count heatmap data with bounds:', bounds, 'Area:', area.toFixed(4))
        ...
        console.log('📊 Count Heatmap RPC result:', { 
        ...
        if (geoError) {
          console.error('❌ Error fetching count heatmap data:', geoError)
        ...
        if (geoData) {
          console.log(`✅ Found count heatmap data`)
          ...
          // Calculate total_sales for each feature (sales_count * avg_price)
          if (parsedData && parsedData.features) {
            parsedData.features.forEach(feature => {
              const props = feature.properties;
              props.total_sales = (props.sales_count || 0) * (props.avg_price || 0);
            });
          }
          
          console.log('📊 Parsed count heatmap data:', {
          ...
          console.warn('⚠️ No count heatmap data found')
          setData(null)
        }
      } catch (err) {
        console.error('💥 Error in fetchCountHeatmapData:', err)
